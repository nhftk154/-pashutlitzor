...
- **חיבור סליקה לחנות (Grow/משולם)** — `shop-worker/worker.js` הוא שלד
  בלבד. ממתין לפתיחת חשבון ב-Grow וקבלת `userId`/`pageCode`/`apiKey` +
  תיעוד API + מפרט אימות ה-callback. פירוט מלא בסעיף
  [הפעלת ה-backend](#הפעלת-ה-backend-חד-פעמי-טרם-בוצע) למטה.
- **שירות שליחת מיילי הזמנה (Resend)** — `shop-worker/worker.js` שולח
  מיילי אישור הזמנה דרך Resend (`RESEND_API_KEY`). ממתין לפתיחת חשבון
  ולקבלת API key. נפרד לגמרי מ-Grow.
- מסמכים משפטיים — ראו "עדיין פתוח" בסעיף
  [מסמכים משפטיים](#מסמכים-משפטיים-מדיניות-פרטיות--הצהרת-נגישות--תנאי-שימוש)
  למטה (הסכמות לתמונות בגלריה, כתוביות לסרטונים).
- **תמונות למוצרי החנות** — `content/products.json` יובא מקטלוג הוואטסאפ
  העסקי (32 מוצרים, שם+מחיר+תיאור קצר), אבל **בלי תמונות** — צילומי המסך
  מהם יובא הקטלוג היו ברזולוציה נמוכה מדי לשימוש באתר. עד שיתווספו תמונות
  אמיתיות לכל מוצר (ב-`/admin`, collection "מוצרים"), האתר יציג placeholder
  (רקע בצבע האתר בלי תמונה) במקומן — לא broken image.
- **6 סדנאות/אירועים מהקטלוג לא יובאו לחנות** — "ערב נשים לכבוד יום
  האישה", "סדנה לנשות מילואים", "סדנה לקהילה", "סדנה לנשים בכל הגילאים",
  "סדנת בת מצווה", "סדנה בתיכון יעדים" — הופיעו בקטלוג הוואטסאפ בלי מחיר
  קבוע (כנראה מתומחרים לפי הזמנה/גודל קבוצה), ולכן לא מתאימים למבנה
  ה-checkout של החנות (שדורש מחיר קבוע מראש). אם רוצים למכור אותם דרך
  האתר צריך להחליט על מודל תמחור קודם.
- **"פמוטים קלאסים בגדלים שונים"** (`content/products.json`, slug
  `candlesticks-classic-sizes`) — בקטלוג המקורי מופיע "גדלים:" בלי פירוט
  מחיר לכל גודל בנפרד. כרגע מוגדר במחיר אחיד (₪35) בלי וריאציות — לתקן
  כשיהיה פירוט מדויק.
...
## מסמכים משפטיים (מדיניות פרטיות | הצהרת נגישות | תנאי שימוש)

שלושה עמודים קיימים ופעילים: `legal/privacy.html`, `legal/accessibility.html`,
`legal/terms.html` — בעיצוב מלא של האתר, עם קישורים מהפוטר. פרטי העסק (שם,
מספר עוסק, כתובת, אימייל) כבר מולאו.

**עדיין פתוח — לזכור להשלים**:

- החלטה לגבי תמונות משתתפות מזוהות בגלריה: האם יש הסכמה בכתב לפרסום, ואם
  לא — איך להתקדם (להשיג הסכמה / להסיר / להחליף). נדחה ביוזמת הבעלים,
  עדיין לא טופל.
- כתוביות/תמלול לסרטוני הסדנאות (`#videos`) — חלק מהסרטונים יישארו בלי
  כתוביות; `legal/accessibility.html` מציע במקום זאת מסלול פנייה לקבלת
  תוכן בפורמט נגיש לפי בקשה, לצד סעיף פטור מחובת הנגשה מכוח מחזור עסקי
  נמוך (לפי תקנות שוויון זכויות לאנשים עם מוגבלות) — **מומלץ לוודא מול
  רואה חשבון/עו"ד את הסף המדויק העדכני לפני הסתמכות מלאה על הפטור**.
