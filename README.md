...
## מסמכים משפטיים (מדיניות פרטיות | הצהרת נגישות | תנאי שימוש) — בעבודה

יש תוכנית מוכנה להוספת שלושה עמודים (`legal/privacy.html`,
`legal/accessibility.html`, `legal/terms.html`) בעיצוב מלא של האתר, עם
קישורים מהפוטר. חלק מהעבודה הטכנית (תיקוני נגישות בקוד: alt תקין, ניגודיות
צבעים) כבר בוצע. **מה שחסר כדי לסיים ולכתוב את התוכן בפועל — להביא בפעם
הבאה**:

- שם העסק המשפטי המלא (איך רשום ברשות המסים)
- מספר עוסק (פטור/מורשה)
- כתובת (לפחות עיר, לצורך ההצהרות)
- אימייל ליצירת קשר בנושאי פרטיות/נגישות (או להשתמש רק במספר הווטסאפ הקיים)
- החלטה לגבי תמונות משתתפות מזוהות בגלריה: האם יש הסכמה בכתב לפרסום, ואם
  לא — איך להתקדם (להשיג הסכמה / להסיר / להחליף)
